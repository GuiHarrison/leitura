...
function enqueue_theme_scripts(): void
{
  ...
  // Página Cashback Leitura
  if (is_page('cashback-leitura')) {
    wp_enqueue_style(
      'cashback-css',
      get_theme_file_uri('css/cashback.css'),
      [],
      filemtime(get_theme_file_path('css/cashback.css'))
    );

    wp_enqueue_script(
      'cashback-core-js',
      get_theme_file_uri('js/src/modules/cashback/core.min.js'),
      ['jquery'],
      filemtime(get_theme_file_path('js/src/modules/cashback/core.min.js')),
      true
    );
    ...
    wp_enqueue_script(
      'cashback-js',
      get_theme_file_uri('js/src/modules/cashback/cashback.js'),
      ['jquery', 'cashback-three-js'],
      filemtime(get_theme_file_path('js/src/modules/cashback/cashback.js')),
      true
    );
...
{
  $env = 'production' === wp_get_environment_type() && ! isset($_GET['load_production_builds']) ? 'dev' : 'prod'; // phpcs:ignore WordPress.Security.NonceVerification.Recommended
