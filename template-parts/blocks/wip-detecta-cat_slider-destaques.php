...
?>
  <section id="destaques-blog" class="grid owl-carousel owl-theme">
    ...
    foreach ($destaque_blog as $post) {
      setup_postdata($post);
      // Verifica se o post está na categoria se-liga-na-leitura ou suas subcategorias
      $is_resenha = false;
      $categories = get_the_category();
      foreach ($categories as $category) {
        if ($category->slug === 'se-liga-na-leitura' || cat_is_ancestor_of(get_category_by_slug('se-liga-na-leitura'), $category)) {
          $is_resenha = true;
          break;
        }
      }
